...
#define OV5640_REG_CLK_ENABLE00		0x3004
#define OV5640_REG_CLK_ENABLE01		0x3005
...
#define OV5640_REG_MCU_FIRMWARE		0x8000

// Auto focus CMD_MAIN register OV5640_REG_AF_CMD_MAIN(0X3022)
#define OV5640_AF_CON_FINISH		0x00
#define OV5640_AF_CON_START			0x03
#define OV5640_AF_CON_STOP			0x06
#define OV5640_AF_CON_FREE			0x08
#define OV5640_AF_CON_RESET_REGION	0x12

// Auto focus CMD_CLK register OV5640_REG_AF_CMD_ACK(0X3023)
#define  OV5640_AF_CON_ACK_FINISHED	0x00
#define  OV5640_AF_CON_ACK_RUNNING	0x01

// Auto focus FW_STATUS register OV5640_REG_AF_FW_STATUS(0X3029)
#define  OV5640_AF_FW_STATUS_S_FIRWARE	0x7f // Firmware downloaded but not running. Because firmware errory or mcu closed.
#define  OV5640_AF_FW_STATUS_S_STARTUP	0x7e // Firmware initialization.
#define  OV5640_AF_FW_STATUS_S_IDLE		0x70 // Free motor and reset focus.
#define  OV5640_AF_FW_STATUS_S_FOCUSING 0x00 // Focusing
#define  OV5640_AF_FW_STATUS_S_FOCUSED  0x10 // Focused
...
struct ov5640_dev {
	...
	bool streaming;
	bool auto_focus_firmware_downloaded;
};
...
static const struct reg_value ov5640_init_setting[] = {
	...
	{0x5387, 0x7c, 0, 0}, {0x5388, 0x6c, 0, 0}, {0x5389, 0x10, 0, 0},// sample
	// {0x5381, 0x20, 0, 0}, {0x5382, 0x64, 0, 0}, {0x5383, 0x08, 0, 0},
	// {0x5384, 0x30, 0, 0}, {0x5385, 0x90, 0, 0}, {0x5386, 0xc0, 0, 0},
	// {0x5387, 0xa0, 0, 0}, {0x5388, 0x98, 0, 0}, {0x5389, 0x08, 0, 0},// default saturation
	// {0x5381, 0x1c, 0, 0}, {0x5382, 0x5a, 0, 0}, {0x5383, 0x06, 0, 0},
	// {0x5384, 0x15, 0, 0}, {0x5385, 0x52, 0, 0}, {0x5386, 0x66, 0, 0},
	// {0x5387, 0x68, 0, 0}, {0x5388, 0x66, 0, 0}, {0x5389, 0x02, 0, 0},// saturation -1
...
static int ov5640_get_af_status(struct ov5640_dev *sensor, u8 *status)
{
	u8 reg = OV5640_AF_FW_STATUS_S_FIRWARE;
	int ret = ov5640_read_reg(sensor, OV5640_REG_AF_FW_STATUS, &reg);
...
// auto focus enable init 600ms finish
static int ov5640_auto_focus_init(struct ov5640_dev *sensor)
{
	struct i2c_client *client = sensor->i2c_client;
	int total_cnt = 0;
	int sent_cnt = 0;
	int ret = 0;
	u16 addr = OV5640_REG_MCU_FIRMWARE;
	u8 temp;

	if (sensor->auto_focus_firmware_downloaded == true)
		return 0;
	dev_dbg(&client->dev, "ov5640 auto focus download firmware\n");
	ov5640_write_reg(sensor, OV5640_REG_SYS_RESET00, 0x20); // Reset MCU

	// download firmware
	total_cnt = ARRAY_SIZE(OV5647_af_firmware);
	dev_dbg(&client->dev, "%s: ov5640 af firmware total count = %d\n", __func__, total_cnt);
	for (sent_cnt = 0; sent_cnt < total_cnt; sent_cnt++) {
		ov5640_write_reg(sensor, addr, OV5647_af_firmware[sent_cnt]);
		addr++;
		}

	// reset auto focus register
	ov5640_write_reg(sensor, 0x3022, 0x00);
	ov5640_write_reg(sensor, 0x3023, 0x00);
	ov5640_write_reg(sensor, 0x3024, 0x00);
	ov5640_write_reg(sensor, 0x3025, 0x00);
	ov5640_write_reg(sensor, 0x3026, 0x00);
	ov5640_write_reg(sensor, 0x3027, 0x00);
	ov5640_write_reg(sensor, 0x3028, 0x00);
	ov5640_write_reg(sensor, 0x3029, 0x7f);
	ov5640_write_reg(sensor, 0x3000, 0x00);

	// MCU check
	ret = ov5640_read_reg(sensor, OV5640_REG_SYS_RESET00, &temp);
	if (ret)
		return ret;
	if (GET_BIT(temp, 5) != 0 || GET_BIT(temp, 6) != 0)
		dev_err(&client->dev, "%s: ov5640 afc download firmware, mcu check failed.\n", __func__);

	ret = ov5640_read_reg(sensor, OV5640_REG_CLK_ENABLE00, &temp);
	if (ret)
		return ret;
	if (GET_BIT(temp, 5) != 1 || GET_BIT(temp, 6) != 1)
		dev_err(&client->dev, "%s: ov5640 afc download firmware, muc check failed.\n", __func__);

	// AFC check
	ret = ov5640_read_reg(sensor, OV5640_REG_SYS_RESET01, &temp);
	if (ret)
		return ret;
	if (GET_BIT(temp, 6) != 0)
		dev_err(&client->dev, "%s: ov5640 afc download firmware, afc check failed.\n", __func__);

	ret = ov5640_read_reg(sensor, OV5640_REG_CLK_ENABLE01, &temp);
	if (ret)
		return ret;
	if (GET_BIT(temp, 6) != 1)
		dev_err(&client->dev, "%s: ov5640 afc download firmware, afc check failed.\n", __func__);
	dev_dbg(&client->dev, "ov5640 auto focus init end\n");
	sensor->auto_focus_firmware_downloaded = true;
	return 0;
}

//  call before stream_on
static int ov5640_auto_focus(struct ov5640_dev *sensor)
{
	int autoFocusTimeoutCnt = 0;
	int ret = 0;
	u8 af_status;

	if (!sensor->auto_focus_firmware_downloaded) {
		dev_err(&sensor->i2c_client->dev, "%s: ov5640 auto focus doesn't downlod.\n", __func__);
		return -1;
	}
	ov5640_get_af_status(sensor, &af_status);
	dev_dbg(&sensor->i2c_client->dev, "%s %d: ov5640 afc start status: %02x\n", __func__, __LINE__, af_status);
	ret = ov5640_write_reg(sensor, OV5640_REG_AF_CMD_MAIN, OV5640_AF_CON_RESET_REGION);
	if (ret)
		return ret;

	msleep(20);
	ret = ov5640_write_reg(sensor, OV5640_REG_AF_CMD_MAIN, OV5640_AF_CON_START);
	if (ret)
		return ret;

	while (autoFocusTimeoutCnt < 20) {
		ov5640_get_af_status(sensor, &af_status);
		if (ret)
			return ret;
		if (af_status == OV5640_AF_FW_STATUS_S_FOCUSED) {
			dev_dbg(&sensor->i2c_client->dev, "%s %d: ov5640 afc end status: %02x\n", __func__, __LINE__, af_status);
			ov5640_write_reg(sensor, OV5640_REG_AF_CMD_MAIN, OV5640_AF_CON_STOP);
			return 0;
		}
		mdelay(100);
		autoFocusTimeoutCnt++;
	}
	dev_err(&sensor->i2c_client->dev, "%s: ov5640 afc timeout.\n", __func__);
	return -1;
}

static int ov5640_set_focus_auto(struct ov5640_dev *sensor)
{
	struct ov5640_ctrls *ctrls = &sensor->ctrls;
	struct i2c_client *client = sensor->i2c_client;
	int ret = 1;

	if (ctrls->focus_auto->val) {
		ret = ov5640_auto_focus_init(sensor); // download fireware
		if (ret)
			return ret;
	} else if (ctrls->focus_auto->val && ctrls->focus_auto->has_changed) {
		dev_info(&client->dev, "%s: ctrls->focus_auto->has_changed: %d, ctrls->focus_auto->val %d.\n", __func__, ctrls->focus_auto->has_changed, ctrls->focus_auto->val);
		ov5640_write_reg(sensor, OV5640_REG_AF_CMD_MAIN, OV5640_AF_CON_FREE);
	} else {
		ret = 0;
	}
...
static int ov5640_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_FOCUS_AUTO:
		ret = ov5640_set_focus_auto(sensor);
		break;
	case V4L2_CID_AUTOGAIN:
...
static int ov5640_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable) {
		dev_dbg(&sensor->i2c_client->dev, "%s %d: ov5640 start capture.\n", __func__, __LINE__);
		...
		if (sensor->ctrls.focus_auto->val) {
			ret = ov5640_auto_focus(sensor);
			if (ret) {
				pm_runtime_put(&sensor->i2c_client->dev);
				return ret;
			}
		...
		if (sensor->ctrls.focus_auto->val) {
			ret = ov5640_write_reg(sensor, OV5640_REG_AF_CMD_MAIN, OV5640_AF_CON_FREE);
			if (ret)
				return ret;
		}
		dev_dbg(&sensor->i2c_client->dev, "\n%s %d: ov5640 stop capture.\n", __func__, __LINE__);
	}
...
static int ov5640_probe(struct i2c_client *client)
{
	...
	sensor->ae_target = 52;
	sensor->auto_focus_firmware_downloaded = false;
	...
	// if (sensor->ctrls.focus_auto->val) {
	ret = ov5640_auto_focus_init(sensor);
	if (ret) {
		dev_err(dev, "failed to init auto focus\n");
		goto free_ctrls;
	}
	// }
