	...
	for (group_idx = 0; group_idx < ISP_SETTINGS_END; group_idx++) {
		...
		if (actual_idx != group_idx) {
			...
			if (DEBUG_REG_BUFF_MESG)
				isp_video_print_reg_buff(ker_rq_addr, ISP_REGQ_SIZE, ISP_SETTINGS_END);
			break;
		} else if (reg_num == 0) {
			printk("%s(%d) !!!write reg buff error!!! \n", __func__, __LINE__);
			printk("%s(%d) reg num is zero (id=%d) \n", __func__, __LINE__, actual_idx);
			...
			for (idx = 0; idx < reg_num; idx++) {
				...
				if (reg_setting->dirty == 1) {
					if (reg_setting->offset > REG_MAX_OFFSET) {
						printk("%s(%d) reg offset out of range = 0x%x \n",
						       __func__, __LINE__, reg_setting->offset);
						continue;
					}
					writel(reg_setting->value, isp_hw_base_addr + reg_setting->offset);
					...
					if (DEBUG_REG_BUFF_MESG) {
						...
						if (real_val != reg_setting->value)
							printk("%s(%d) isp reg diff !!! offset(0x%x) real = 0x%x  ideal = 0x%x  \n",
							       __func__, __LINE__, reg_setting ->offset, real_val, reg_setting ->value);
					}
				}
...
void isp_video_print_reg_buff(u8 *usr_reg_addr, size_t buf_size, size_t group_size)
{
	...
	for (group_idx = 0; group_idx < group_size; group_idx++) {
		...
		for (idx = 0; idx < reg_num; idx++) {
			...
			printk("%x %x %x\n", reg_setting->dirty, reg_setting->offset, reg_setting->value);
