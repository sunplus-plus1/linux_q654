...
static u32 isp_video_set_3dnr_addr(struct isp_video_fh *vfh, struct cap_buff *cap_bufq,
								   struct out_buff *out_bufq,
								   struct statis_buff *y_buf,
								   struct statis_buff *uv_buf,
								   struct statis_buff *v_buf,
...
static void isp_video_reset_statis(struct isp_video_fh *vfh, struct ModuleRest *reset_module)
{
	...
	if (((reset_module->reset_all == 1) ||
		...
		((func_en_0 & FUNC_EN_0_FCURVE_EN_MASK) ==
	...
	if (((reset_module->reset_all == 1) ||
		...
		((func_en_1 & FUNC_EN_1_WDR_EN_MASK) ==
		 FUNC_EN_1_WDR_EN_MASK))
	{
	...
	if (((reset_module->reset_all == 1) ||
		...
		((func_en_1 & FUNC_EN_1_LCE_ENABLE_MASK) ==
	...
	if (((reset_module->reset_all == 1) ||
		...
		((func_en_1 & FUNC_EN_1_CNR_ENABLE_MASK) ==
	...
	if (((reset_module->reset_all == 1) ||
		...
		((func_en_1 & FUNC_EN_1_3DNR_ENABLE_MASK) ==
		 FUNC_EN_1_3DNR_ENABLE_MASK))
	{
...
static long isp_video_reg_no_setting(size_t cap_width, size_t cap_height)
{
	...
	WRITE_REG(isp_cur_base, ISP_CONTROL_0, (cap_height << 16) | cap_width,
			  0xFFFFFFFF);
	WRITE_REG(isp_cur_base, ISP_SCALER_0, (cap_height << 16) | cap_width,
			  0xFFFFFFFF);
	WRITE_REG(isp_cur_base, ISP_SCALER_1, (cap_height << 16) | cap_width,
			  0xFFFFFFFF);
	return 0;
}
...
static long isp_video_write_reg(struct isp_video_fh *vfh, size_t cap_width, size_t cap_height)
{
	...
	list_for_each_safe(listptr, listptr_next, &vfh->usr_reg_list) {
		...
		if (entry->usr_header->input_sequence == in_buf_sequence) {
			...
			if (DEBUG_BUFF_MESG)
				printk("(isp)  %s(%d) get handle=0x%px  statis_id=%lld  \n", __func__, __LINE__, vfh, entry->usr_header->statis_id);
			...
			if (DEBUG_BUFF_MESG) {
				printk("(isp) ori_reset flag: all=%d fcurve=%d wdr=%d lce=%d cnr=%d 3dnr=%d  %s(%d)\n",
					   ...
					   entry->usr_header->reset_module.reset_3dnr, __func__, __LINE__);
			}
			isp_video_reset_statis(vfh, &entry->usr_header->reset_module);
			if (DEBUG_BUFF_MESG) {
				printk("(isp) after_reset flag: all=%d fcurve=%d wdr=%d lce=%d cnr=%d 3dnr=%d  %s(%d)\n",
					   entry->usr_header->reset_module.reset_all,
					   entry->usr_header->reset_module.reset_fcurve,
					   entry->usr_header->reset_module.reset_wdr,
					   entry->usr_header->reset_module.reset_lce,
			...
			break;
		} else if (entry->usr_header->input_sequence < in_buf_sequence) {
...
{
	// launch
	WRITE_REG(isp_cur_base, ISP_START_LAUNCH, START_LAUNCH_FRAME_START_MASK,
			  START_LAUNCH_FRAME_START_MASK);
}
...
static long isp_video_reg_buf_add(struct isp_video_fh *handle, u32 input_data)
{
	...
	if (DEBUG_BUFF_MESG)
	{
		...
		printk("(isp)  %s(%d)  statis_id=%lld  \n", __func__,
			   __LINE__, usr_setting->header.statis_id);
	}
...
static void set_statis_write_phy_addr(struct isp_video_fh *vfh, enum ISP_DMA_ID e_module)
{
	int readIdx = 0;
	int writeIdx = 0;
	...
	case CAP_WDR:
		...
		if (writeIdx == 0)
			...
			WRITE_REG(isp_cur_base, ISP_DMA_12, phy_addr + max_size,
					  0xFFFFFFFF)
	...
	case CAP_LCE:
		...
		if (writeIdx == 0)
			...
			WRITE_REG(isp_cur_base, ISP_DMA_16, phy_addr + max_size,
					  0xFFFFFFFF)
		break;
	case CAP_CNR:
		...
		if (writeIdx == 0)
			...
			WRITE_REG(isp_cur_base, ISP_DMA_20, phy_addr + max_size,
					  0xFFFFFFFF)
		break;
	default:
		break;
...
static u32 isp_video_set_3dnr_flow(struct isp_video_fh *vfh, struct statis_buff *y_buf)
{
	...
	if (first_en == 1)
	{
		...
		WRITE_REG(isp_cur_base, ISP_FUNC_EN_1, FUNC_EN_1_3DNR_FIRST_FRAME_MASK,
	...
	if (((fun_ctrl_2 & FUNC_EN_2_HSCALE_EN_MASK) ==
		 FUNC_EN_2_HSCALE_EN_MASK) ||
		((fun_ctrl_2 & FUNC_EN_2_VSCALE_EN_MASK) ==
		 FUNC_EN_2_VSCALE_EN_MASK))
		sc_en = 1;
	y_buf->sc_en = sc_en;
								   ...
								   struct statis_buff *mot_buf)
{
	...
	if ((y_buf->phy_addr != 0) || (last_y_addr != 0))
	{
		if (last_y_addr != 0)
			...
			WRITE_REG(isp_cur_base, ISP_DMA_22, y_buf->phy_addr,
					  0xFFFFFFFF)
	...
	if (out_sep_en == 0)
	{
		...
		{
			if (last_uv_addr != 0)
				WRITE_REG(isp_cur_base, ISP_DMA_26, last_uv_addr,
						  0xFFFFFFFF)
			else
				WRITE_REG(isp_cur_base, ISP_DMA_26, uv_buf->phy_addr,
						  0xFFFFFFFF)
		}
	...
	else
	{
		...
		if ((uv_buf->phy_addr != 0) || (last_uv_addr != 0))
		{
			if (last_uv_addr != 0)
			{
				WRITE_REG(isp_cur_base, ISP_DMA_49, last_uv_addr,
						  0xFFFFFFFF)
				WRITE_REG(isp_cur_base, ISP_DMA_50, last_v_addr,
						  0xFFFFFFFF)
			}
			else
			{
				WRITE_REG(isp_cur_base, ISP_DMA_49, uv_buf->phy_addr,
						  0xFFFFFFFF)
				WRITE_REG(isp_cur_base, ISP_DMA_50, v_buf->phy_addr,
						  0xFFFFFFFF)
			}
		}
...
static void set_statis_read_phy_addr(struct isp_video_fh *vfh, enum ISP_DMA_ID e_module)
{
	...
	case OUT_FCURVE:
	{
		...
		WRITE_REG(isp_cur_base, ISP_FUNC_EN_0, FUNC_EN_0_FCURVE_SUBOUT_EN_MASK,
		...
		else
		{
			...
			if (readIdx == 0)
				WRITE_REG(isp_cur_base, ISP_DMA_4, phy_addr,
						  0xFFFFFFFF)
			else
				WRITE_REG(isp_cur_base, ISP_DMA_4, phy_addr + max_size,
						  0xFFFFFFFF)
	...
	case OUT_WDR:
	{
		...
		// wdr subout enable
		WRITE_REG(isp_cur_base, ISP_FUNC_EN_1, FUNC_EN_1_WDR_SUBOUT_EN_MASK,
		...
		else
		{
			...
			if (readIdx == 0)
				WRITE_REG(isp_cur_base, ISP_DMA_10, phy_addr,
						  0xFFFFFFFF)
			else
				WRITE_REG(isp_cur_base, ISP_DMA_10,
						  phy_addr + max_size, 0xFFFFFFFF)

			// wdr enable
			WRITE_REG(isp_cur_base, ISP_FUNC_EN_1, FUNC_EN_1_WDR_EN_MASK,
					  FUNC_EN_1_WDR_EN_MASK);
		}
	}
	break;
	...
	case OUT_LCE:
	{
		...
		else
		{
			...
			if (readIdx == 0)
				WRITE_REG(isp_cur_base, ISP_DMA_14, phy_addr,
						  0xFFFFFFFF)
			else
				WRITE_REG(isp_cur_base, ISP_DMA_14,
						  phy_addr + max_size, 0xFFFFFFFF)
	...
	case OUT_CNR:
	{
		...
		else
		{
			...
			if (readIdx == 0)
				WRITE_REG(isp_cur_base, ISP_DMA_18, phy_addr,
						  0xFFFFFFFF)
			else
				WRITE_REG(isp_cur_base, ISP_DMA_18,
						  phy_addr + max_size, 0xFFFFFFFF)
...
static void isp_video_free_3dnr(struct isp_video_fh *handle)
{
	...
	if (handle->hw_buff.d3nr_y_buff.max_size != 0)
	{
		...
		if (virt_addr)
		{
			...
			buff_size = ISP_PAGE_ALIGN(
				handle->hw_buff.d3nr_y_buff.max_size *
				2);
			dma_free_coherent(handle->video->isp_dev->dev,
							  buff_size, virt_addr,
							  phy_addr);
	...
	if (handle->hw_buff.d3nr_uv_buff.max_size != 0)
	{
		...
		{
			phy_addr =
				handle->hw_buff.d3nr_uv_buff.phy_addr;
			phy_addr_out = handle->hw_buff.d3nr_uv_buff
							   .phy_addr_out;
	...
	if (handle->hw_buff.d3nr_mot_buff.max_size != 0)
	{
		...
		{
			phy_addr =
				handle->hw_buff.d3nr_mot_buff.phy_addr;
			phy_addr_out = handle->hw_buff.d3nr_mot_buff
							   .phy_addr_out;
			...
			handle->hw_buff.d3nr_mot_buff.virt_addr_out =
				NULL;
		}
...
static int isp_video_alloc_3dnr(struct isp_video_fh *handle, int sc_en)
{
		...
		(handle->hw_buff.d3nr_mot_buff.max_size == 0))
	{
		if (!D3NR_REDUCE_BANDWITH || (sc_en != 0))
		{
			...
			buff_size = ISP_PAGE_ALIGN(
				handle->hw_buff.d3nr_y_buff.max_size * 2);
			...
			buff_size = ISP_PAGE_ALIGN(
				...
				2);
			virt_addr =
				dma_alloc_coherent(handle->video->isp_dev->dev,
								   buff_size, &phy_addr,
								   GFP_KERNEL);
			if (virt_addr == NULL)
			{
			...
			handle->hw_buff.d3nr_uv_buff.phy_addr =
				phy_addr;
			handle->hw_buff.d3nr_uv_buff.virt_addr =
				virt_addr;
			...
			if (out_sep_mode != 0)
			{
				...
				handle->hw_buff.d3nr_v_buff.virt_addr =
					virt_addr;
		...
		buff_size = ISP_PAGE_ALIGN(
			handle->hw_buff.d3nr_mot_buff.max_size * 2);
		virt_addr = dma_alloc_coherent(handle->video->isp_dev->dev,
									   buff_size, &phy_addr,
									   GFP_KERNEL);
		...
		handle->hw_buff.d3nr_mot_buff.phy_addr_out =
			phy_addr +
			handle->hw_buff.d3nr_mot_buff.max_size;
		handle->hw_buff.d3nr_mot_buff.virt_addr_out =
			virt_addr +
		...
		if (DEBUG_BUFF_MESG)
		{
			printk("3dnr alloc, handle=0x%px, in_w=%d, in_h=%d, y max_size = 0x%lx,u max_size = 0x%lx, v max_size = 0x%lx, mot max_size = 0x%lx\n ",
				   handle, width, height,
				   handle->hw_buff.d3nr_y_buff.max_size,
				   handle->hw_buff.d3nr_uv_buff.max_size,
				   handle->hw_buff.d3nr_v_buff.max_size,
...
#define FS_GAINL_MAX (16384)
	...
	fs_index_wt_bayer =
		(fs_index_wt_bayer & HDR_8_FS_INDEX_WT_BAYER_MASK) >>
		HDR_8_FS_INDEX_WT_BAYER_LSB;
	if (fs_index_wt_bayer > fs_index_wt_bayer_max)
	{
		printk("(isp) fs_index_wt_bayer is out of range, val = %d (0-%d)  %s(%d)\n",
			   fs_index_wt_bayer, fs_index_wt_bayer_max, __func__,
			   __LINE__);
	}
	...
	fs_high_thres = (fs_low_thres & HDR_10_FS_HIGH_THRES_MASK) >>
					HDR_10_FS_HIGH_THRES_LSB;
...
#define SUBIN_FLIT_MAX (9)
	...
	block_h = (block_w & FCURVE_0_FCURVE_SUB_HEIGHT_MASK) >>
			  FCURVE_0_FCURVE_SUB_HEIGHT_LSB;
	...
	scaling_fact_v =
		(scaling_fact_h & FCURVE_3_FCURVE_SCALING_FACT_V_MASK) >>
		FCURVE_3_FCURVE_SCALING_FACT_V_LSB;
	...
	blk_size_v = (blk_size_h & FCURVE_4_FCURVE_BLK_SIZE_V_MASK) >>
				 FCURVE_4_FCURVE_BLK_SIZE_V_LSB;
...
#define MLSC_BLK_MAX (64)
	...
	mlsc_lut_num_y = (mlsc_lut_num_x & MLSC_1_MLSC_LUT_NUM_Y_MASK) >>
					 MLSC_1_MLSC_LUT_NUM_Y_LSB;
	mlsc_lut_num_x = (mlsc_lut_num_x & MLSC_1_MLSC_LUT_NUM_X_MASK);

	if ((mlsc_lut_num_y > mlsc_blk_max) ||
		(mlsc_lut_num_y < mlsc_blk_min))
	{
		printk("(isp) mlsc_lut_num_y is out of range, val = %d (%d-%d)  %s(%d)\n",
			   mlsc_lut_num_y, mlsc_blk_min, mlsc_blk_max, __func__,
			   __LINE__);
		ret = -1;
	}

	if ((mlsc_lut_num_x > mlsc_blk_max) ||
		(mlsc_lut_num_x < mlsc_blk_min))
	{
		printk("(isp) mlsc_lut_num_x is out of range, val = %d (%d-%d)  %s(%d)\n",
			   mlsc_lut_num_x, mlsc_blk_min, mlsc_blk_max, __func__,
			   __LINE__);
		ret = -1;
	}
...
#define BNR_WT_NLM_MAX (32)
	...
	bnr_snr_filt_w3 = (bnr_snr_filt_w0 & BNR_0_BNR_SNR_FILT_W3_MASK) >>
					  BNR_0_BNR_SNR_FILT_W3_LSB;
	bnr_snr_filt_w2 = (bnr_snr_filt_w0 & BNR_0_BNR_SNR_FILT_W2_MASK) >>
					  BNR_0_BNR_SNR_FILT_W2_LSB;
	bnr_snr_filt_w1 = (bnr_snr_filt_w0 & BNR_0_BNR_SNR_FILT_W1_MASK) >>
					  BNR_0_BNR_SNR_FILT_W1_LSB;
	bnr_snr_filt_w0 = (bnr_snr_filt_w0 & BNR_0_BNR_SNR_FILT_W0_MASK);

	bnr_snr_filt_w5 = (bnr_snr_filt_w4 & BNR_1_BNR_SNR_FILT_W5_MASK) >>
					  BNR_1_BNR_SNR_FILT_W5_LSB;
	...
	bnr_patch_norm_div =
		(bnr_patch_norm_div & BNR_2_BNR_PATCH_NORM_DIV_MASK) >>
		BNR_2_BNR_PATCH_NORM_DIV_LSB;
...
#define WDR_Y_MAX (8)
	...
	block_h = (block_w & WDR_0_WDR_SUB_HEIGHT_MASK) >>
			  WDR_0_WDR_SUB_HEIGHT_LSB;
	...
	scaling_fact_v = (scaling_fact_h & WDR_3_WDR_SCALING_FACT_V_MASK) >>
					 WDR_3_WDR_SCALING_FACT_V_LSB;
	...
	blk_size_v = (blk_size_h & WDR_4_WDR_BLK_SIZE_V_MASK) >>
				 WDR_4_WDR_BLK_SIZE_V_LSB;
	...
	subin_filt_norm_div =
		(subin_flit_w0 & WDR_6_WDR_SUBIN_FILT_NORM_DIV_MASK) >>
	...
	subin_flit_w1 = (subin_flit_w0 & WDR_6_WDR_SUBIN_FILT_W1_MASK) >>
					WDR_6_WDR_SUBIN_FILT_W1_LSB;
	subin_flit_w2 = (subin_flit_w0 & WDR_6_WDR_SUBIN_FILT_W2_MASK) >>
					WDR_6_WDR_SUBIN_FILT_W2_LSB;
...
#define SUBOUT_BLK_H_MAX (32)
	...
	block_h = (block_w & LCE_0_LCE_SUB_HEIGHT_MASK) >>
			  LCE_0_LCE_SUB_HEIGHT_LSB;
	...
	scaling_fact_v = (scaling_fact_h & LCE_3_LCE_SCALING_FACT_V_MASK) >>
					 LCE_3_LCE_SCALING_FACT_V_LSB;
	...
	blk_size_v = (blk_size_h & LCE_4_LCE_BLK_SIZE_V_MASK) >>
				 LCE_4_LCE_BLK_SIZE_V_LSB;
	...
	subin_filt_norm_div =
		(subin_flit_w0 & LCE_6_LCE_SUBIN_FILT_NORM_DIV_MASK) >>
	...
	subin_flit_w1 = (subin_flit_w0 & LCE_6_LCE_SUBIN_FILT_W1_MASK) >>
					LCE_6_LCE_SUBIN_FILT_W1_LSB;
	subin_flit_w2 = (subin_flit_w0 & LCE_6_LCE_SUBIN_FILT_W2_MASK) >>
					LCE_6_LCE_SUBIN_FILT_W2_LSB;
...
#define CNR_SUBOUT_BLK_H_MAX (511 * 2)
	...
	if ((frame_h / 4) < block_h_max)
		block_h_max = frame_h / 4;

	if ((frame_w / 4) < block_w_max)
		block_w_max = frame_w / 4;
	...
	scaling_fact_v = (scaling_fact_h & CNR_3_CNR_SCALING_FACT_V_MASK) >>
					 CNR_3_CNR_SCALING_FACT_V_LSB;
	...
	blk_size_v = (blk_size_h & CNR_4_CNR_BLK_SIZE_V_MASK) >>
				 CNR_4_CNR_BLK_SIZE_V_LSB;
...
#define SP_WT_MAX (32)
	...
	sp_sad_clamp_inv_norm =
		(sp_sad_clamp_val & SP_4_SP_SAD_CLAMP_INV_NORM_MASK) >>
	...
	if (sp_sad_clamp_val == 0 && sp_sad_clamp_inv_norm != (1 << 15))
	{
		printk("(isp)  invalid val, sp_sad_clamp_val=0x%x , sp_sad_clamp_inv_norm=0x%x %s(%d)\n",
			   sp_sad_clamp_val, sp_sad_clamp_inv_norm, __func__,
			   __LINE__);
		ret = -1;
	}
	...
	sp_wt_low_main_sub =
		(sp_wt_low_main_sub & SP_5_SP_WT_LOW_MAIN_SUB_MASK) >>
		SP_5_SP_WT_LOW_MAIN_SUB_LSB;
	...
	sp_wt_low_diredge = (sp_wt_low_diredge & SP_6_SP_WT_LOW_DIREDGE_MASK) >>
						SP_6_SP_WT_LOW_DIREDGE_LSB;
	...
	sp_wt_low_2d_flat = (sp_wt_low_2d_flat & SP_7_SP_WT_LOW_2D_FLAT_MASK) >>
						SP_7_SP_WT_LOW_2D_FLAT_LSB;
	...
	sp_wt_high_main_sub =
		(sp_wt_high_main_sub & SP_5_SP_WT_HIGH_MAIN_SUB_MASK) >>
		SP_5_SP_WT_HIGH_MAIN_SUB_LSB;
	...
	sp_wt_high_diredge =
		(sp_wt_high_diredge & SP_6_SP_WT_HIGH_DIREDGE_MASK) >>
		SP_6_SP_WT_HIGH_DIREDGE_LSB;
	...
	sp_wt_high_2d_flat =
		(sp_wt_high_2d_flat & SP_7_SP_WT_HIGH_2D_FLAT_MASK) >>
		SP_7_SP_WT_HIGH_2D_FLAT_LSB;
...
#define NR3D_SAT_RATIO_MAX (64)
	...
	nr3d_sf_filt_w1 = (nr3d_sf_filt_w0 & NR3D_23_NR3D_SF_FILT_W1_MASK) >>
					  NR3D_23_NR3D_SF_FILT_W1_LSB;
	nr3d_sf_filt_w2 = (nr3d_sf_filt_w0 & NR3D_23_NR3D_SF_FILT_W2_MASK) >>
					  NR3D_23_NR3D_SF_FILT_W2_LSB;
	nr3d_sf_filt_w3 = (nr3d_sf_filt_w0 & NR3D_23_NR3D_SF_FILT_W3_MASK) >>
					  NR3D_23_NR3D_SF_FILT_W3_LSB;
	nr3d_sf_filt_w0 = (nr3d_sf_filt_w0 & NR3D_23_NR3D_SF_FILT_W0_MASK);

	nr3d_sf_filt_w5 = (nr3d_sf_filt_w4 & NR3D_24_NR3D_SF_FILT_W5_MASK) >>
					  NR3D_24_NR3D_SF_FILT_W5_LSB;
...
static void isp_reg_check_all(void)
{
	...
	// fusion
	if ((func_en_0 & FUNC_EN_0_HDR_FUSION_EN_MASK) ==
		FUNC_EN_0_HDR_FUSION_EN_MASK)
	{
	...
	if (((func_en_0 & FUNC_EN_0_FCURVE_EN_MASK) ==
		 FUNC_EN_0_FCURVE_EN_MASK) ||
		((func_en_0 & FUNC_EN_0_FCURVE_SUBOUT_EN_MASK) ==
		 FUNC_EN_0_FCURVE_SUBOUT_EN_MASK))
	{
	...
	if ((func_en_0 & FUNC_EN_0_BNR_EN_MASK) == FUNC_EN_0_BNR_EN_MASK)
	{
		if (isp_reg_check_bnr() == -1)
			WRITE_REG(isp_cur_base, ISP_FUNC_EN_0, 0x0,
					  FUNC_EN_0_BNR_EN_MASK);
	}

	// wdr
	if (((func_en_1 & FUNC_EN_1_WDR_EN_MASK) == FUNC_EN_1_WDR_EN_MASK) ||
		((func_en_1 & FUNC_EN_1_WDR_SUBOUT_EN_MASK) ==
		 FUNC_EN_1_WDR_SUBOUT_EN_MASK))
	{
		if (isp_reg_check_wdr() == -1)
		{
			...
			WRITE_REG(isp_cur_base, ISP_FUNC_EN_1, 0x0,
					  FUNC_EN_1_WDR_SUBOUT_EN_MASK);
			WRITE_REG(isp_cur_base, ISP_FUNC_EN_1, 0x0,
					  FUNC_EN_1_WDR_DIFF_CONTROL_EN_MASK);
			WRITE_REG(isp_cur_base, ISP_FUNC_EN_1, 0x0,
					  FUNC_EN_1_WDR_OUTBLD_EN_MASK);
		}
	}

	// lce
	if (((func_en_1 & FUNC_EN_1_LCE_ENABLE_MASK) ==
		 FUNC_EN_1_LCE_ENABLE_MASK) ||
		((func_en_1 & FUNC_EN_1_LCE_SUBOUT_ENABLE_MASK) ==
		 FUNC_EN_1_LCE_SUBOUT_ENABLE_MASK))
	{
	...
	// cnr
	if (((func_en_1 & FUNC_EN_1_CNR_ENABLE_MASK) ==
		 FUNC_EN_1_CNR_ENABLE_MASK) ||
		((func_en_1 & FUNC_EN_1_CNR_SUBOUT_ENABLE_MASK) ==
		 FUNC_EN_1_CNR_SUBOUT_ENABLE_MASK))
	{
	...
	// sp
	if ((func_en_1 & FUNC_EN_1_SP_ENABLE_MASK) ==
		FUNC_EN_1_SP_ENABLE_MASK)
	{
	...
	// 3dnr
	if ((func_en_1 & FUNC_EN_1_3DNR_ENABLE_MASK) ==
		FUNC_EN_1_3DNR_ENABLE_MASK)
	{
...
static long isp_video_reg_handle(struct isp_video_fh *vfh, struct cap_buff *cap_bufq,
								 ...
								 size_t cap_height)
{
	...
	{
		WRITE_REG(isp_cur_base, ISP_DMA_0, (u32)out_bufq->input_hdr1_buf, 0xFFFFFFFF);
	}
	else if (out_bufq->input_rgbir_size != 0)
	{
		WRITE_REG(isp_cur_base, ISP_DMA_0, (u32)out_bufq->input_rgbir_buf, 0xFFFFFFFF);
	}

	if (out_bufq->input_hdr2_size != 0)
		WRITE_REG(isp_cur_base, ISP_DMA_2, (u32)out_bufq->input_hdr2_buf, 0xFFFFFFFF);
	...
	if ((func_en_1 & FUNC_EN_1_3DNR_ENABLE_MASK) == FUNC_EN_1_3DNR_ENABLE_MASK)
	{
		func_en_1 = isp_video_set_3dnr_addr(vfh, cap_bufq, out_bufq,
											&(vfh->hw_buff.d3nr_y_buff),
	...
	if (cap_bufq->v_out_size == 0)
	{
		if (cap_bufq->y_out_size != 0)
			WRITE_REG(isp_cur_base, ISP_DMA_30, (u32)cap_bufq->y_out_buff,
					  0xFFFFFFFF);

		if (cap_bufq->u_out_size != 0)
			WRITE_REG(isp_cur_base, ISP_DMA_32, (u32)cap_bufq->u_out_buff,
					  0xFFFFFFFF);
	}
	else
	{
		...
		if (cap_bufq->u_out_size != 0)
			WRITE_REG(isp_cur_base, ISP_DMA_47, (u32)cap_bufq->u_out_buff,
					  0xFFFFFFFF);

		if (cap_bufq->v_out_size != 0)
			WRITE_REG(isp_cur_base, ISP_DMA_48, (u32)cap_bufq->v_out_buff,
					  0xFFFFFFFF);
	}
...
static int isp_initialize_modules(struct platform_device *pdev,
								  struct isp_device *isp_dev)
{
	...
	if (!request_mem_region(res_haps.start, resource_size(&res_haps),
							np_dev->full_name))
	{
		ret = -ENOMEM;
		of_node_put(np_dev);
								  ...
								  struct device *alloc_devs[])
{
	...
	*num_planes = vfh->out_format.fmt.pix_mp.num_planes;
	...
	{
		sizes[idx] =
			vfh->out_format.fmt.pix_mp.plane_fmt[idx].sizeimage;
	}
	return 0;
}
...
static void isp_video_set_cap_addr(struct isp_video_fh *vfh, struct cap_buff *cap_buf,
								   ...
								   struct vb2_v4l2_buffer *v4l2_buf)
{
	...
	case PLANE_Y_UV:
	{
		...
		if ((V4L2_PIX_FMT_NV12M == data_fmt) ||
			(V4L2_PIX_FMT_NV16M == data_fmt))
		{
			// y / uv
		...
		else
		{
			...
			{
				cap_buf->awb_out_buff =
					vb2_dma_contig_plane_dma_addr(buf, 1) + 0x100;
	...
	case PLANE_Y_UV_STATIS:
	{
		...
		{
			cap_buf->awb_out_buff =
				vb2_dma_contig_plane_dma_addr(buf, 2) + 0x100;
	...
	case PLANE_Y_U_V_STATIS:
	{
		...
		{
			cap_buf->awb_out_buff =
				vb2_dma_contig_plane_dma_addr(buf, 3) + 0x100;
	...
	case PLANE_Y_UV_3D_STATIS:
		...
		cap_buf->d3nr_Y_ref_out_buff =
			vb2_dma_contig_plane_dma_addr(buf, 2);
		cap_buf->d3nr_UV_ref_out_buff =
			vb2_dma_contig_plane_dma_addr(buf, 3);
		cap_buf->d3nr_motion_cur_out_buff =
			vb2_dma_contig_plane_dma_addr(buf, 4);
		...
		{
			cap_buf->awb_out_buff =
				vb2_dma_contig_plane_dma_addr(buf, 5) + 0x100;
	...
	case PLANE_Y_U_V_3D_STATIS:
		...
		cap_buf->d3nr_UV_ref_out_buff =
			vb2_dma_contig_plane_dma_addr(buf, 4);
		cap_buf->d3nr_motion_cur_out_buff =
			vb2_dma_contig_plane_dma_addr(buf, 5);
		...
		{
			cap_buf->awb_out_buff =
				vb2_dma_contig_plane_dma_addr(buf, 6) + 0x100;
	...
	{
		cap_buf->ae_ba_buff =
			cap_buf->awb_out_buff + alloc_statis_buf_size[0];
		cap_buf->ae_hist_buff =
			cap_buf->ae_ba_buff + alloc_statis_buf_size[1];
		cap_buf->ae2_ba_buff =
			cap_buf->ae_hist_buff + alloc_statis_buf_size[2];
		cap_buf->ae2_hist_buff =
			cap_buf->ae2_ba_buff + alloc_statis_buf_size[3];

		cap_buf->ae_ba_virt =
			cap_buf->awb_virt + alloc_statis_buf_size[0];
		cap_buf->ae_hist_virt =
			cap_buf->ae_ba_virt + alloc_statis_buf_size[1];
		cap_buf->ae2_ba_virt =
			cap_buf->ae_hist_virt + alloc_statis_buf_size[2];
		cap_buf->ae2_hist_virt =
			cap_buf->ae2_ba_virt + alloc_statis_buf_size[3];
...
static void isp_fill_statis_info(struct isp_video_fh *vfh, struct cap_buff *cap_buf)
{
	...
	if (awb_out_size > 0 && statis_dma != NULL)
	{
		...
		statis_info_addr->awb_statis.awb_max_size =
			alloc_statis_buf_size[0];
		...
		statis_info_addr->ae1_ba_statis.ae_ba_max_size =
			alloc_statis_buf_size[1];
		statis_info_addr->ae1_ba_statis.data_offset =
			statis_info_addr->awb_statis.data_offset +
			alloc_statis_buf_size[0];
		...
		statis_info_addr->ae1_hist_statis.ae_hist_max_size =
			alloc_statis_buf_size[2];
		statis_info_addr->ae1_hist_statis.data_offset =
			statis_info_addr->ae1_ba_statis.data_offset +
			alloc_statis_buf_size[1];

		statis_info_addr->ae2_ba_statis.enable = ae2_ba_en;
		statis_info_addr->ae2_ba_statis.ae_ba_max_size =
			alloc_statis_buf_size[3];
		...
		statis_info_addr->ae2_hist_statis.ae_hist_max_size =
			alloc_statis_buf_size[4];
		statis_info_addr->ae2_hist_statis.data_offset =
			statis_info_addr->ae2_ba_statis.data_offset +
			alloc_statis_buf_size[3];

		dma_buf_vunmap(statis_dma, statis_vbuf);
...
/* crop */
static int isp_g_selection(struct file *file, void *fh,
...
static int isp_s_selection(struct file *file, void *fh,
						   struct v4l2_selection *sel)
{
...
static int isp_get_out_format(struct file *file, void *fh,
							  struct v4l2_format *format)
{
	return 0;
}

static int isp_get_out_format_mplane(struct file *file, void *fh,
									 struct v4l2_format *format)
{
	...
	for (idx = 0; idx < format->fmt.pix_mp.num_planes; idx++)
	{
		...
		format->fmt.pix_mp.plane_fmt[idx].bytesperline =
			vfh->out_format.fmt.pix_mp.plane_fmt[idx].bytesperline;
	}
	return 0;
}

static int isp_get_cap_format(struct file *file, void *fh,
							  struct v4l2_format *format)
{
	return 0;
}

static int isp_get_cap_format_mplane(struct file *file, void *fh,
									 struct v4l2_format *format)
{
	struct isp_video_fh *vfh = to_isp_video_fh(fh);
	int idx = 0;
...
static int isp_set_out_format_mplane(struct file *file, void *fh,
									 struct v4l2_format *format)
{
	struct isp_video_fh *vfh = to_isp_video_fh(fh);
	int idx = 0;
	...
	if (DEBUG_FORMAT_MESG)
	{
		...
		printk("(isp) out plane num = %d (%s)(%d)  \n",
			   vfh->out_format.fmt.pix_mp.num_planes, __func__,
			   __LINE__);
	}
	...
	for (idx = 0; idx < vfh->out_format.fmt.pix_mp.num_planes; idx++)
	{
		...
		if (DEBUG_FORMAT_MESG)
		{
			...
			printk("(isp) plane size = 0x%x (%s)(%d)  \n",
				   vfh->out_format.fmt.pix_mp.plane_fmt[idx]
					   .sizeimage,
				   __func__, __LINE__);
...
static int isp_set_cap_format_mplane(struct file *file, void *fh,
									 struct v4l2_format *format)
{
	struct isp_video_fh *vfh = to_isp_video_fh(fh);
	int idx = 0;
...
static int isp_video_set_format_output(struct file *file, void *fh,
									   struct v4l2_format *format)
{
	struct isp_video_fh *vfh = to_isp_video_fh(fh);
...
static int isp_video_enum_out_fmt(struct file *file, void *priv,
								  struct v4l2_fmtdesc *f)
{
	if (f->index == 0)
	{
...
static int isp_video_try_cap_fmt(struct file *file, void *priv,
...
static int isp_video_try_out_fmt(struct file *file, void *priv,
								 struct v4l2_format *f)
{
	struct v4l2_pix_format_mplane *pix_fmt_mp = &f->fmt.pix_mp;
	pix_fmt_mp->field = V4L2_FIELD_NONE;
...
static int isp_video_enum_framesizes(struct file *file, void *priv,
...
static void isp_video_set_out_addr(struct out_buff *out_buf,
								   struct vb2_buffer *buf)
{
	int mplane_num = buf->num_planes;
...
static void isp_video_hw_setting(struct work_struct *work)
{
	struct isp_video_fh *vfh = container_of((void *)work, struct isp_video_fh, work);
	struct v4l2_m2m_dev *m2m_dev = NULL;
	...
	isp_video_reg_handle(vfh, &cap_buf, &out_buf, cap_width,
						 cap_height);
	...
	if (INTR_TIMEOUT_MODE)
		timeout =
			wait_event_interruptible_timeout(isp_hw_done_wait,
											 isp_hw_done_flags > 0,
											 (100 * HZ) / 1000);
	else
		wait_event_interruptible_exclusive(isp_hw_done_wait,
										   isp_hw_done_flags > 0);
...
static void isp_release_dma_buf(struct isp_video_fh *handle)
{
	...
	if (virt_addr && (handle->hw_buff.fcurve_buff.sram_en == 0))
	{
		...
		buff_size = ISP_PAGE_ALIGN(
			handle->hw_buff.fcurve_buff.max_size * 2);
		dma_free_coherent(handle->video->isp_dev->dev, buff_size, virt_addr,
						  phy_addr);
	...
	if (virt_addr && (handle->hw_buff.wdr_buff.sram_en == 0))
	{
		...
		buff_size =
			ISP_PAGE_ALIGN(handle->hw_buff.wdr_buff.max_size * 2);
		dma_free_coherent(handle->video->isp_dev->dev, buff_size, virt_addr,
						  phy_addr);
	...
	if (virt_addr && (handle->hw_buff.lce_buff.sram_en == 0))
	{
		...
		buff_size =
			ISP_PAGE_ALIGN(handle->hw_buff.lce_buff.max_size * 2);
		dma_free_coherent(handle->video->isp_dev->dev, buff_size, virt_addr,
						  phy_addr);
	...
	if (virt_addr && (handle->hw_buff.cnr_buff.sram_en == 0))
	{
		...
		buff_size =
			ISP_PAGE_ALIGN(handle->hw_buff.cnr_buff.max_size * 2);
		dma_free_coherent(handle->video->isp_dev->dev, buff_size, virt_addr,
						  phy_addr);
	...
	// sram unmap
	if (IS_ISP_SRAM_EN(handle->regQ_id) && (handle->isp_sram_virt != NULL))
	{
...
static int isp_alloc_dma_buf(struct isp_video_fh *handle)
{
	...
	u32 width = 0;
	u32 height = 0;
	size_t sram_size[3] = {0}; // SRAM arrangement: fcurve size, wdr size, lce size
	...
	if (IS_ISP_SRAM_EN(handle->regQ_id))
	{
		...
		handle->isp_sram_virt = ioremap(isp_sram_phy + handle->regQ_id * handle->sram_size, handle->sram_size);
	...
	if (IS_ISP_SRAM_EN(handle->regQ_id))
	{
		...
		{
			handle->hw_buff.fcurve_buff.phy_addr = isp_sram_phy + handle->regQ_id * handle->sram_size;
	...
	{
		virt_addr = dma_alloc_coherent(handle->video->isp_dev->dev, buff_size,
									   &phy_addr, GFP_KERNEL);
	...
	if (IS_ISP_SRAM_EN(handle->regQ_id))
	{
		if (handle->isp_sram_virt != NULL)
		{
			handle->hw_buff.wdr_buff.phy_addr = isp_sram_phy + handle->regQ_id * handle->sram_size + sram_size[0];
	...
	if (IS_ISP_SRAM_EN(handle->regQ_id))
	{
		if (handle->isp_sram_virt != NULL)
		{
			handle->hw_buff.lce_buff.phy_addr = isp_sram_phy + handle->regQ_id * handle->sram_size + sram_size[0] + sram_size[1];
			handle->hw_buff.lce_buff.virt_addr = handle->isp_sram_virt + sram_size[0] + sram_size[1];
	...
	if (DEBUG_BUFF_MESG)
	{
		printk("cnr buff addr = 0x%px , handle=0x%px, in_w=%d, in_h=%d, max_size=0x%lx\n",
			   handle->hw_buff.cnr_buff.virt_addr, handle,
			   width, height,
			   handle->hw_buff.cnr_buff.max_size);
	}
...
static int isp_register_video_dev(int id, struct isp_device *isp)
{
	int r;
...
enum ISP_CLK_GATE_STATUS isp_video_get_clk_func_status(void)
{
	if (CLK_GATING_FUNC_EN)
		if (isp_drv_clk != NULL)
			if (isp_dbg_clk_status != ISP_CLK_FUNC_DIS)
...
static int isp_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MAX_VIDEO_DEVICE; i++)
	{
		...
		ret = isp_register_video_dev(i, isp[i]);
		if (ret < 0)
			goto error_register;
	}
...
#ifdef CONFIG_PM_RUNTIME_VPU_DEFAULT
		SET_RUNTIME_PM_OPS(isp_rtpm_suspend_ops, isp_rtpm_resume_ops, NULL)
#endif
};
#endif
