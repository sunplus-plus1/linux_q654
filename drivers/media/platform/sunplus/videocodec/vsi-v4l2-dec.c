...
static int vsi_dec_qbuf(struct file *filp, void *priv, struct v4l2_buffer *buf)
{
	...
	v4l2_klog(LOGLVL_FLOW, "%lx:%s:%d:%d:%d:%d", ctx->ctxid, __func__, buf->type, buf->index, buf->bytesused, buf->length);
	if (!vsi_v4l2_daemonalive())
		return -ENODEV;
	if (!isvalidtype(buf->type, ctx->flag))
		return -EINVAL;
	...
	if (ret != 0)
		v4l2_klog(LOGLVL_FLOW, "%s:fail with %d", __func__, ret);
				  ...
				  struct v4l2_frmsizeenum *fsize)
{
	...
	fmt.fmt.pix.pixelformat = fsize->pixel_format;
	...
	v4l2_klog(LOGLVL_CONFIG, "%s:%x->%d:%d:%d:%d:%d:%d", __func__, fsize->pixel_format,
...
static int vsi_dec_setup_ctrls(struct v4l2_ctrl_handler *handler)
{
	...
	for (i = 0; i < ctrl_num; i++) {
		...
		if (handler->error) {
			v4l2_klog(LOGLVL_WARNING, "%s fail to set ctrl %d:%d", __func__, i, handler->error);
			handler->error = 0;
		}
	}
