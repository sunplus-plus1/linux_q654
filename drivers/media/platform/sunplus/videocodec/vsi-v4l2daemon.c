...
static int hasMsg(void)
{
	int id, ret = 0;
	struct vsi_v4l2_msg *obj;

	while (!mutex_trylock(&cmd_lock))
		cpu_relax();
...
static void format_bufinfo_dec(struct vsi_v4l2_ctx *ctx, struct vsi_v4l2_msg *pmsg, struct vb2_buffer *buf)
{
	...
	if (!binputqueue(buf->type)) {
		...
		decbufinfo->OutBufSize = vbuf->planes[0].length;
		decbufinfo->bytesused = vbuf->planes[0].bytesused;
...
int vsiv4l2_execcmd(struct vsi_v4l2_ctx *ctx, enum v4l2_daemon_cmd_id id, void *args)
{
	...
	case V4L2_DAEMON_VIDIOC_ENC_RESET:
		ret = vsi_v4l2_sendcmd(id, ctx->ctxid,
			ctx->mediacfg.m_encparams.m_codecfmt.codecFormat, NULL, &retflag, 0, 0);
		break;
	...
	case V4L2_DAEMON_VIDIOC_STREAMON:
		if (test_and_clear_bit(CTX_FLAG_ENC_FLUSHBUF, &ctx->flag))
			param = 1;
		ret = vsi_v4l2_sendcmd(id, ctx->ctxid,
			ctx->mediacfg.m_encparams.m_codecfmt.codecFormat, NULL, &retflag, 0, param);
		break;
	case V4L2_DAEMON_VIDIOC_STREAMOFF_OUTPUT:
		if (isencoder(ctx))
			ret = vsi_v4l2_sendcmd(id, ctx->ctxid,
				ctx->mediacfg.m_encparams.m_srcinfo.inputFormat, NULL, &retflag, 0, 0);
		else
			ret = vsi_v4l2_sendcmd(id, ctx->ctxid,
	...
	case V4L2_DAEMON_VIDIOC_BUF_RDY:
		if (isencoder(ctx)) {
			...
			size = format_bufinfo_enc(ctx, &msg, args);
			ret = vsi_v4l2_sendcmd(id, ctx->ctxid, ctx->mediacfg.m_encparams.m_codecfmt.codecFormat,
					&msg.params, &retflag, size, 0);
		} else {
...
static ssize_t v4l2_msg_read(struct file *fh, char __user *buf, size_t size, loff_t *offest)
{
	...
	ret = wait_event_interruptible_timeout(cmd_queue, (hasMsg() != 0), msecs_to_jiffies(100));
	if (ret == -ERESTARTSYS)
		return -EIO;
	else if (ret == 0)
		return 0;
	return getMsg(fh, buf, size);
}
