...
#include <linux/clk.h>
#include <dt-bindings/regulator/sunplus,sp7350-regulator.h>
...
static int reg_iso_remove(struct fixed_voltage_data *priv)
{
	unsigned long flags;
	u32 value;
	u32 iso;
...
static int reg_iso_apply(struct fixed_voltage_data *priv)
{
	unsigned long flags;
	u32 value;
	u32 iso;
...
static int reg_iso_enable(struct regulator_dev *rdev)
{
	struct fixed_voltage_data *priv = rdev_get_drvdata(rdev);
	int ret;

	ret = reg_iso_remove(priv);
	if (ret)
		return ret;
...
static int reg_iso_disable(struct regulator_dev *rdev)
{
	struct fixed_voltage_data *priv = rdev_get_drvdata(rdev);
	int ret;

	ret = reg_iso_apply(priv);
	if (ret)
		return ret;
...
static const struct regulator_ops fixed_voltage_iso_ops = {
	.enable = reg_iso_enable,
	.disable = reg_iso_disable,
	.is_enabled = reg_is_enabled,
};
...
static int reg_fixed_voltage_probe(struct platform_device *pdev)
{
	...
	} else if (drvtype && drvtype->has_enable_iso) {
		drvdata->desc.ops = &fixed_voltage_iso_ops;
...
static const struct fixed_dev_type fixed_iso_data = {
	.has_enable_iso = true,
};
...
MODULE_DESCRIPTION("Sunplus Fixed voltage regulator");
MODULE_LICENSE("GPL");
