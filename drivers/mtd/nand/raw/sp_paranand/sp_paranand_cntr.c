...
static inline void sp_nfc_set_row_col_addr(
		struct sp_nfc *nfc, int row, int col)
{
	int val;
...
int sp_nfc_issue_cmd(struct nand_chip *chip, struct cmd_feature *cmd_f)
{
	...
	if (status == 0) {
		sp_nfc_set_row_col_addr(nfc, cmd_f->row_cycle, cmd_f->col_cycle);
...
int sp_nfc_wait(struct nand_chip *chip)
{
	...
	//No command
	if (readl(nfc->regs + CMDQUEUE_STATUS) & CMDQUEUE_STATUS_EMPTY(nfc->cur_chan)) {
	...
	do {
		...
		if (likely(cmd_comp_sts & (1 << (nfc->cur_chan)))) {
			...
			if (nfc->read_state) {
				ecc_intr_sts = readl(nfc->regs + ECC_INTR_STATUS);
				// Clear the ECC intr status
				writel(ecc_intr_sts, nfc->regs + ECC_INTR_STATUS);
				// ECC failed on nfc
				...
				if (ecc_sts_for_spare & (1 << nfc->cur_chan)) {
					nfc->cmd_status |= CMD_ECC_FAIL_ON_SPARE;
					ret = NAND_STATUS_FAIL;
				}
			}
			goto out;
...
void sp_nfc_fill_prog_code(struct nand_chip *chip, int location,
		int cmd_index) {
	struct sp_nfc *nfc = nand_get_controller_data(chip);
				...
				u_char *spare_buf)
{
	...
	for(i = 0; i < tmp_len; i += nfc->max_spare) {
		...
		cmd_f.cq2 = CMD_EX_SPARE_NUM(cmd_len) | SCR_SEED_VAL2(nfc->seed_val);
		...
		for(j = 0; j < cmd_len + 1; j += 4) {
			if (j / 4 == cmd_len / 4) {
				memcpy(buf + i + j, nfc->regs + SPARE_SRAM + j, cmd_len % 4);
				break;
			}
...
int sp_nfc_check_bad_spare(struct nand_chip *chip, int pg)
{
	...
	if(!byte_rd(chip, pg, spare_phy_start, spare_phy_len, spare_buf)) {
		...
		if (errbit_num != 0) {
			if (nfc->cur_chan < 4) {
				chan = (nfc->cur_chan << 3);
				ecc_corr_bit_spare = (readl(nfc->regs + ECC_CORRECT_BIT_FOR_SPARE_REG1) >> chan) & 0x7F;
			}
			else {
				chan = (nfc->cur_chan - 4) << 3;
				ecc_corr_bit_spare = (readl(nfc->regs + ECC_CORRECT_BIT_FOR_SPARE_REG2) >> chan) & 0x7F;
			}
			DBGLEVEL1(sp_nfc_dbg("spare_phy_len = %d, errbit_num = %d\n", spare_phy_len, errbit_num));
...
int rd_pg_by_dma(struct nand_chip *chip, int real_pg, uint8_t *data_buf)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	struct sp_nfc *nfc = nand_get_controller_data(chip);
	...
	if(ret < 0) {
		printk("sp_nfc_issue_cmd failed!\n");
		goto err_unmap_buf;
	}
...
int wr_pg_by_dma(struct nand_chip *chip, int real_pg, const uint8_t *data_buf)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	struct sp_nfc *nfc = nand_get_controller_data(chip);
	struct nand_ecc_ctrl *ecc = &chip->ecc;
		...
		uint8_t *buf, int oob_required, int page)
{
	...
	DBGLEVEL2(sp_nfc_dbg
		...
		nfc->cur_chan, nfc->sel_chip, nfc->page_addr, real_pg, mtd->writesize, nfc->column));
	...
	if (nfc->cmd_status &
		...
		if(chk_spare_0xFF == 1) {
			...
			//~lichun
				if(sp_nfc_check_bad_spare(chip, nfc->page_addr)) {
					printk(KERN_ERR "ECC err for spare(Read page) @");
					printk(KERN_ERR	"ch:%d ce:%d page0x%x real:0x%x\n",
						nfc->cur_chan, nfc->sel_chip, nfc->page_addr, real_pg);
					spare_empty = 0;
...
int sp_nfc_write_page(struct nand_chip *chip, const uint8_t *buf,
...
int sp_nfc_read_oob(struct nand_chip *chip, int page)
{
	...
	DBGLEVEL2(sp_nfc_dbg(
		...
		nfc->cur_chan, nfc->sel_chip, nfc->page_addr, real_pg, mtd->writesize));
...
int sp_nfc_write_oob(struct nand_chip *chip, int page)
{
	...
	DBGLEVEL2(sp_nfc_dbg(
		...
		nfc->cur_chan, nfc->sel_chip, nfc->page_addr, real_pg, mtd->writesize, mtd->oobsize));
